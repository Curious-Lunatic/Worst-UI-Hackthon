...
function buildKeyboard() {
  ...
  keys.forEach(key => {
    ...
    keyElem.dataset.key = key === 'CLEAR' ? 'CLEAR' : key;
    keyElem.textContent = key;
    if (key === 'CLEAR') keyElem.id = 'clear-btn';
...
document.addEventListener("mousemove", e => {
  ...
  if (fanPos.x < -fanElem.clientWidth/2) fanPos.x = -fanElem.clientWidth/2;
  else if (fanPos.x > document.body.clientWidth - fanElem.clientWidth/2) fanPos.x = document.body.clientWidth - fanElem.clientWidth/2;
  if (fanPos.y < -fanElem.clientHeight/2) fanPos.y = -fanElem.clientHeight/2;
  else if (fanPos.y > document.body.clientHeight - fanElem.clientHeight/2) fanPos.y = document.body.clientHeight - fanElem.clientHeight/2;
});
...
function blowCursor(deltaTime) {
  ...
  const force = (FAN_SPEED / 10) * (FAN_SPEED / 10) / distance;
  cursorVel.x += force * direction.x * deltaTime;
  cursorVel.y += force * direction.y * deltaTime;
}

function decelerateCursor(deltaTime) {
  const accX = 0.1 * cursorVel.x * cursorVel.x / 2;
  const accY = 0.1 * cursorVel.y * cursorVel.y / 2;
  if (Math.round(cursorVel.x * 1000) == 0) cursorVel.x = 0;
  else cursorVel.x += accX * (cursorVel.x > 0 ? -1 : 1);
  if (Math.round(cursorVel.y * 1000) == 0) cursorVel.y = 0;
  else cursorVel.y += accY * (cursorVel.y > 0 ? -1 : 1);
}
...
function updateCursor(deltaTime) {
  ...
  else if (cursorPos.x > document.body.clientWidth - cursorElem.clientWidth) cursorPos.x = document.body.clientWidth - cursorElem.clientWidth;
  if (cursorPos.y < 0) cursorPos.y = 0;
  else if (cursorPos.y > document.body.clientHeight - cursorElem.clientHeight) cursorPos.y = document.body.clientHeight - cursorElem.clientHeight;
...
function handleCursorHover() {
  for (const key of keyElems) {
    if (isCollision(cursorElem.getBoundingClientRect(), key.getBoundingClientRect())) {
      ...
      lastKey = key;
      if (key.id === 'clear-btn') textElem.innerText = '';
      else textElem.innerText += key.dataset.key;
      key.classList.add('hover');
      return;
    }
  }
  if (lastKey) lastKey.classList.remove('hover');
  lastKey = null;
}
